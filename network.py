...
class RAM():
    ...
    def __init__(self, PARAMETERS, session):
        ...
        with tensorflow.variable_scope(scope, reuse=tensorflow.AUTO_REUSE):
            ...
            # Create LSTM Cell
            self.lstm_cell_1 = tensorflow.nn.rnn_cell.LSTMCell(self.hs_size, activation=tensorflow.nn.relu, state_is_tuple=True, name="cell_1")

            # Tensorflow Placeholder
            self.actions = tensorflow.placeholder(shape=self.batch_size, dtype=tensorflow.int32)
            self.actions_onehot = tensorflow.one_hot(self.actions, self.output_dim, dtype=tensorflow.float32)
            self.input_hs_1 = tensorflow.placeholder(tensorflow.float32, shape=(2, self.batch_size, self.hs_size))
            self.glances = tensorflow.placeholder(shape=(), dtype=tensorflow.float32)
            self.loss_list_b = tensorflow.placeholder(shape=(), dtype=tensorflow.float32)
            self.input_pressure= tensorflow.placeholder(tensorflow.float32, shape=(self.batch_size,self.sensor_size), name="pressure")
            self.input_location= tensorflow.placeholder(tensorflow.float32, shape=(self.batch_size,2), name="location")
            self.input_mean_x = tensorflow.placeholder(tensorflow.float32, shape=(self.batch_size,1), name="mean_x")
            self.input_mean_a = tensorflow.placeholder(tensorflow.float32, shape=(self.batch_size,1), name="mean_a")
            self.input_std_x = tensorflow.placeholder(tensorflow.float32, shape=(self.batch_size,1), name="std_x")
            self.input_std_a = tensorflow.placeholder(tensorflow.float32, shape=(self.batch_size,1), name="std_a")
